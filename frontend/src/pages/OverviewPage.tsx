...
export default function OverviewPage() {
  ...
  const wifiSummaryItems = wifiSummaryQuery.data?.items ?? [];

  const activeWifiAlerts = wifiAlertsQuery.data ?? [];

  const weakestWifiRoom = getWeakestWifiRoom(wifiSummaryItems);

  const weakestWifiRoomActiveAlert = weakestWifiRoom
    ? (activeWifiAlerts.find(
        (alert) => alert.entity_key === weakestWifiRoom.location_label,
      ) ?? null)
    : null;
  ...
  const criticalAlerts = (criticalAlertsQuery.data ?? []).filter(
  ...
  const activeUnacknowledgedCriticalCount =
    overview?.alerts.active_unacknowledged_critical_count ?? 0;

  const shouldShowCriticalBanner = activeUnacknowledgedCriticalCount > 0;
  ...
  const [alertsFocusMode, setAlertsFocusMode] = useState<
    ...
    },
    {
      ...
      ...(wifiSummaryQuery.isLoading
        ? {
            ...
            detail: "Waiting for Wi-Fi room summaries",
          }
        : wifiSummaryQuery.isError
          ? {
              status: "error" as const,
              detail: "Wi-Fi summary request failed",
            }
          : wifiSummaryItems.length > 0
            ...
            : {
                status: "ok" as const,
                detail: "No Wi-Fi room summaries yet",
              }),
    },
  ];
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      <PageFilterBar
        title="Overview"
        description="Current network health, active incidents, and the next signals worth reviewing."
        controls={null}
      >
        {overview ? (
          <span className="rounded-full border border-zinc-800 bg-zinc-950 px-3 py-1 text-xs text-zinc-300">
            Last check · {formatDate(overview.checked_at)}
          </span>
        ) : null}

        <span className="rounded-full border border-zinc-800 bg-zinc-950 px-3 py-1 text-xs text-zinc-300">
          Active alerts · {overview ? overview.alerts.active_count : "—"}
        </span>

        <span className="rounded-full border border-zinc-800 bg-zinc-950 px-3 py-1 text-xs text-zinc-300">
          Ongoing outages · {overview ? overview.outages.active_count : "—"}
        </span>
      </PageFilterBar>
        ...
        <section className="rounded-2xl border border-red-900 bg-red-950/40 px-4 py-4 sm:px-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              ...
              {topCriticalAlert ? (
                <>
                  <p className="mt-1 text-base font-medium text-zinc-100">
                    {buildAlertHeadline({
                      entityType: topCriticalAlert.entity_type,
                      entityKey: topCriticalAlert.entity_key,
                  ...
                  <p className="mt-1 text-xs text-zinc-400">
                    {formatIncidentType(topCriticalAlert.entity_type)} · Opened{" "}
                    {formatDate(topCriticalAlert.created_at)}
                  </p>
                </>
              ) : isCriticalBannerPreviewLoading ? (
                <>
                  <p className="mt-1 text-base font-medium text-zinc-100">
                    Loading critical alert details...
                  </p>
                  <p className="mt-1 text-sm text-zinc-300">
                    Alert preview is being refreshed.
                  </p>
                </>
              ) : (
                <>
                  ...
                  <p className="mt-1 text-sm text-zinc-300">
                    Preview details are temporarily unavailable.
                  </p>
                </>
              )}
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              <span className="rounded-full border border-red-800 bg-red-950 px-2.5 py-1 text-xs text-red-300">
                {activeUnacknowledgedCriticalCount} critical unacknowledged
              </span>

              <button
                type="button"
                onClick={() => {
                  setSelectedOverviewAlertId(null);
                  setAlertsFocusMode("active-critical");
                  scrollElementIntoView(alertsSectionRef.current);
                }}
              ...
              {topCriticalAlert ? (
                <button
                  ...
                  onClick={() => {
                    setAlertsFocusMode("active-critical");
                    setSelectedOverviewAlertId(topCriticalAlert.id);
                    scrollElementIntoView(alertsSectionRef.current);
                  }}
          ...
          {isCriticalBannerPreviewUnavailable ? (
            <p className="mt-3 text-xs text-red-200/80">
              Critical alert details are temporarily unavailable.
            </p>
          ) : null}
        </section>
      ) : null}
      ...
      <section className="space-y-4">
        <div>
          <h3 className="text-lg font-medium">Attention now</h3>
          <p className="mt-1 text-sm text-zinc-400">
            The fastest read on current service health and incident pressure.
          </p>
        </div>
        ...
        <div className="pt-1">
          ...
          <p className="mt-1 text-sm text-zinc-400">
            Recent activity, outage pressure, and latest dashboard refresh
            context.
          </p>
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <StatCard
            ...
            value={
              overview
                ? String(overview.outages.active_count)
                : overviewQuery.isLoading
                  ? "Loading"
                  : "—"
            }
          ...
          <StatCard
            ...
            value={
              overview
                ? String(overview.devices.active_count_24h)
                : overviewQuery.isLoading
                  ? "Loading"
                  : "—"
            }
            hint={
              overview
                ? `Last seen ${formatDate(
                    overview.devices.most_recent_seen_at,
                  )}`
                : "Waiting for overview data"
            }
          />
          ...
          <StatCard
            ...
            value={
              trafficSummary
                ? formatBytes(trafficSummary.total_bytes)
            ...
            hint={
              trafficSummary
                ? `${trafficSummary.interface_count} interface${
                    trafficSummary.interface_count === 1 ? "" : "s"
                  } observed`
          ...
          <StatCard
            ...
            value={
              trafficSummary?.top_talker
                ? formatInterfaceName(trafficSummary.top_talker.interface_name)
                : trafficSummaryQuery.isLoading
                  ? "Loading"
                  : "—"
            }
            hint={
              trafficSummary?.top_talker
                ? `${formatBytes(
                    trafficSummary.top_talker.delta_bytes_total,
                  )} moved in last hour`
                : trafficSummaryQuery.isError
          ...
          <StatCard
            ...
            value={
              overview
                ? String(overview.alerts.active_count)
                : overviewQuery.isLoading
                  ? "Loading"
                  : "—"
            }
      ...
      <section className="space-y-4">
        <div>
          <h3 className="text-lg font-medium">Current network state</h3>
          <p className="mt-1 text-sm text-zinc-400">
            Latest probe health for router reachability, internet access, and
            DNS.
          </p>
        </div>
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-medium">Router</h3>
              {overview ? (
                <StatusBadge
                  ok={overview.router.is_healthy}
                  activeOutage={overview.router.active_outage}
                />
              ) : null}
            </div>
            ...
            <div className="mt-4 space-y-2 text-sm text-zinc-300">
              <p>
                ...
                <span className="text-zinc-400">
                  {formatDate(overview?.router.last_success_at)}
                </span>
              </p>
              <p>
                Last failure:{" "}
                <span className="text-zinc-400">
                  {formatDate(overview?.router.last_failure_at)}
                </span>
              </p>
              <p>
                Latest latency:{" "}
                <span className="text-zinc-400">
                  {formatMs(overview?.router.latest_latency_ms)}
                </span>
              </p>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5">
            ...
            <div className="mt-4 space-y-2 text-sm text-zinc-300">
              <p>
                Last success:{" "}
                <span className="text-zinc-400">
                  {formatDate(overview?.internet.last_success_at)}
                </span>
              </p>
              <p>
                Last failure:{" "}
                <span className="text-zinc-400">
                  {formatDate(overview?.internet.last_failure_at)}
                </span>
              </p>
              <p>
                Latest latency:{" "}
                <span className="text-zinc-400">
                  {formatMs(overview?.internet.latest_latency_ms)}
                </span>
              </p>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-medium">DNS</h3>
              {overview ? (
                <StatusBadge
                  ok={overview.dns.is_healthy}
                  activeOutage={overview.dns.active_outage}
                />
              ) : null}
            </div>
            ...
            <div className="mt-4 space-y-2 text-sm text-zinc-300">
              <p>
                Last success:{" "}
                <span className="text-zinc-400">
                  {formatDate(overview?.dns.last_success_at)}
                </span>
              </p>
              <p>
                Last failure:{" "}
                <span className="text-zinc-400">
                  {formatDate(overview?.dns.last_failure_at)}
                </span>
              </p>
              <p>
                Latest response:{" "}
                <span className="text-zinc-400">
                  {formatMs(overview?.dns.latest_response_time_ms)}
                </span>
              </p>
            </div>
          </div>
      ...
      <section className="space-y-4">
        <div>
          <h3 className="text-lg font-medium">Wi-Fi health</h3>
          <p className="mt-1 text-sm text-zinc-400">
            Fast read on room-level Wi-Fi signal quality and sample freshness
            from the last hour.
          </p>
        </div>
        ...
        {wifiSummaryQuery.isLoading ? (
          ...
          <InspectionHighlightCard
            ...
            subtitle={
              weakestWifiRoom?.latest_sample?.ssid
                ? `${weakestWifiRoom.latest_sample.ssid} · sampled ${formatMinutesAgo(
                    weakestWifiRoom.latest_sample.sampled_at,
                  )}`
                : "Room summaries available"
            }
            statusLabel={weakestWifiRoom?.latest_sample?.band ?? undefined}
            statusBadgeClassName="border-zinc-700 bg-zinc-800 text-zinc-300"
            primaryLabel="Weakest signal"
            primaryValue={formatRssi(weakestWifiRoom?.latest_sample?.rssi_dbm)}
            metrics={[
              {
            ...
            footerLabel="Latest Wi-Fi refresh"
            footerValue={
              weakestWifiRoom?.latest_sample?.sampled_at
                ? formatMinutesAgo(weakestWifiRoom.latest_sample.sampled_at)
                : "—"
            }
          ...
          >
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div className="rounded-xl border border-zinc-800 bg-zinc-950/60 p-3">
                ...
                <div className="mt-2 text-sm text-zinc-100">
                  {weakestWifiRoom ? weakestWifiRoom.location_label : "—"}
                </div>
              </div>
                  ...
                  <button
                    ...
                    onClick={() => {
                      const params = new URLSearchParams({
                        location: weakestWifiRoom.location_label,
                        minutes: "60",
                      });

                      if (weakestWifiRoomActiveAlert?.id) {
                        params.set(
                          "alert",
                          String(weakestWifiRoomActiveAlert.id),
                        );
                      }

                      navigate(`/wifi?${params.toString()}`);
                    }}
                    className="rounded-lg border border-zinc-700 px-3 py-2 text-sm text-zinc-200 hover:bg-zinc-800"
                  >
                    Open weakest room
      ...
      <section className="space-y-4">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-400">
            Review active issues and work through alerts that need action.
          </p>
        </div>

        <div ref={alertsSectionRef} className="grid gap-4 lg:grid-cols-2">
      ...
      <section className="space-y-4">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-400">
            Lower-priority API request visibility for local troubleshooting.
          </p>
        </div>
