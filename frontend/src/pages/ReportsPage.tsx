    ...
    <div className="space-y-4">
      ...
      <div className="overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900">
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {outagesQuery.isLoading && outages.length === 0 ? (
              <tr>
                <td colSpan={7} className="px-4 py-6 text-zinc-400">Loading outages...</td>
              </tr>
            ) : outages.length === 0 ? (
              <tr>
              ...
              outages.map((outage) => (
                <tr key={outage.id} className="border-t border-zinc-800">
                  ...
                  <td className="px-4 py-3">{formatDate(outage.started_at)}</td>
                  <td className="px-4 py-3">{formatDate(outage.ended_at)}</td>
