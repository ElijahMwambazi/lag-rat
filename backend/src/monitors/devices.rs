...
pub fn parse_inventory_line(line: &str) -> Option<(String, Option<String>, Option<String>)> {
    parse_linux_proc_arp(line).or_else(|| parse_unix_arp(line)).or_else(|| parse_windows_arp(line))
}
...
pub fn parse_linux_proc_arp(line: &str) -> Option<(String, Option<String>, Option<String>)> {
    ...
    if !looks_like_ipv4(ip) { return None; }
...
fn normalize_mac(value: &str) -> Option<String> {
    ...
    if value.is_empty() || value == "*" || value == "incomplete" || value == "<incomplete>" || value == "(incomplete)" || value == "00:00:00:00:00:00" {
        None
    } else {
...
fn normalize_host(value: &str) -> Option<String> {
    ...
    if value.is_empty() || value == "?" || value == "on" { None } else { Some(value.to_string()) }
}
