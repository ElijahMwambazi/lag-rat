...
use crate::models::{Alert, ConnectivityCheck, Device, DnsCheck, KnownDevice, Outage, SummaryResponse, TimeseriesPoint};
...
pub async fn run_migrations(pool: &SqlitePool) -> anyhow::Result<()> {
    ...
    for entry in entries {
        let path = entry.path();
        if path.extension().and_then(|ext| ext.to_str()) != Some("sql") { continue; }
        let version = match path.file_name().and_then(|n| n.to_str()) { Some(v) => v.to_string(), None => continue };
        let already_applied = sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM schema_migrations WHERE version = ?1")
            .bind(&version).fetch_one(pool).await?;
        if already_applied > 0 { continue; }
        ...
        sqlx::query("INSERT INTO schema_migrations (version, applied_at) VALUES (?1, ?2)")
            .bind(&version).bind(Utc::now()).execute(&mut *tx).await?;
        tx.commit().await?;
    }
    Ok(())
}
...
pub async fn seed_default_known_devices(pool: &SqlitePool, router_ip: &str) -> anyhow::Result<()> {
    let existing: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM known_devices WHERE ip_address = ?1 AND label = ?2")
        .bind(router_ip).bind("Router").fetch_one(pool).await?;
    if existing == 0 {
        let now = Utc::now();
...
pub async fn insert_connectivity_check(pool: &SqlitePool, timestamp: DateTime<Utc>, target: &str, target_type: &str, success: bool, latency_ms: Option<f64>, error_message: Option<&str>) -> anyhow::Result<()> {
...
pub async fn insert_dns_check(pool: &SqlitePool, timestamp: DateTime<Utc>, domain: &str, resolver: &str, success: bool, response_time_ms: Option<f64>, error_message: Option<&str>) -> anyhow::Result<()> {
...
async fn upsert_outage_state(pool: &SqlitePool, outage_type: &str, target: &str, success: bool, timestamp: DateTime<Utc>, error_message: Option<&str>) -> anyhow::Result<()> {
    ...
    match (success, active) {
        ...
        (true, Some(outage)) => {
            sqlx::query("UPDATE outages SET ended_at = ?1, is_active = 0, end_note = ?2 WHERE id = ?3")
                .bind(timestamp).bind("recovered").bind(outage.id).execute(pool).await?;
        }
        _ => {}
    }
    Ok(())
...
pub async fn connectivity_timeseries(pool: &SqlitePool, target_type: &str, minutes: i64) -> anyhow::Result<Vec<TimeseriesPoint>> {
    ...
    Ok(rows.into_iter().map(|row| TimeseriesPoint { timestamp: row.get("timestamp"), value: row.get("latency_ms") }).collect())
}
...
pub async fn dns_timeseries(pool: &SqlitePool, minutes: i64) -> anyhow::Result<Vec<TimeseriesPoint>> {
    ...
    Ok(rows.into_iter().map(|row| TimeseriesPoint { timestamp: row.get("timestamp"), value: row.get("response_time_ms") }).collect())
}
...
pub async fn summary_24h(pool: &SqlitePool) -> anyhow::Result<SummaryResponse> {
    ...
    let outage_row = sqlx::query("SELECT COUNT(*) AS total FROM outages WHERE started_at >= datetime('now', '-24 hours')")
        .fetch_one(pool).await?;
    let outage_count_24h: i64 = outage_row.get("total");

    Ok(SummaryResponse {
        uptime_pct_24h: if total > 0 { (successes.unwrap_or(0) as f64 / total as f64) * 100.0 } else { 0.0 },
        avg_latency_ms_24h: avg_latency.unwrap_or(0.0),
        outage_count_24h: outage_count_24h as u32,
    })
}

pub async fn upsert_device(pool: &SqlitePool, ip_address: &str, mac_address: Option<&str>, hostname: Option<&str>, observed_at: DateTime<Utc>) -> anyhow::Result<()> {
...
pub async fn latest_connectivity_check(pool: &SqlitePool, target_type: &str) -> anyhow::Result<Option<ConnectivityCheck>> {
...
pub async fn last_successful_connectivity_check(pool: &SqlitePool, target_type: &str) -> anyhow::Result<Option<ConnectivityCheck>> {
...
pub async fn last_failed_connectivity_check(pool: &SqlitePool, target_type: &str) -> anyhow::Result<Option<ConnectivityCheck>> {
...
pub async fn active_outages_count(pool: &SqlitePool) -> anyhow::Result<u32> {
    Ok(sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM outages WHERE is_active = 1").fetch_one(pool).await? as u32)
}

pub async fn active_outage_exists(pool: &SqlitePool, outage_type: &str, target: &str) -> anyhow::Result<bool> {
    Ok(sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM outages WHERE outage_type = ?1 AND target = ?2 AND is_active = 1")
        .bind(outage_type).bind(target).fetch_one(pool).await? > 0)
}

pub async fn outage_count_since_hours(pool: &SqlitePool, hours: i64) -> anyhow::Result<u32> {
    Ok(sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM outages WHERE started_at >= datetime('now', '-' || ?1 || ' hours')")
...
pub async fn most_recent_device_seen(pool: &SqlitePool) -> anyhow::Result<Option<DateTime<Utc>>> {
    let row = sqlx::query("SELECT last_seen FROM devices WHERE last_seen IS NOT NULL ORDER BY last_seen DESC LIMIT 1")
        .fetch_optional(pool).await?;
    match row { Some(row) => Ok(Some(row.get("last_seen"))), None => Ok(None) }
}
...
pub async fn upsert_alert_state(pool: &SqlitePool, alert_type: &str, severity: &str, entity_type: &str, entity_key: &str, message: &str, is_active: bool, timestamp: DateTime<Utc>) -> anyhow::Result<()> {
    ...
    match (is_active, existing) {
        ...
        (false, Some(alert)) => {
            sqlx::query("UPDATE alerts SET is_active = 0, resolved_at = ?1 WHERE id = ?2")
                .bind(timestamp).bind(alert.id).execute(pool).await?;
        }
        _ => {}
    }
    Ok(())
