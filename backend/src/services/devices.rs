...
pub async fn list_enriched(state: &AppState) -> anyhow::Result<Vec<EnrichedDevice>> {
    ...
    Ok(devices.into_iter().map(|device| {
        let known = match_known_device(&known_devices, Some(device.ip_address.as_str()), device.mac_address.as_deref());
        let is_recent = device.last_seen.map(|ts| (Utc::now() - ts).num_hours() < 24).unwrap_or(false);
        ...
        let display_name = label.clone().or_else(|| device.hostname.clone()).unwrap_or_else(|| device.ip_address.clone());
...
fn match_known_device<'a>(known_devices: &'a [KnownDevice], ip_address: Option<&str>, mac_address: Option<&str>) -> Option<&'a KnownDevice> {
