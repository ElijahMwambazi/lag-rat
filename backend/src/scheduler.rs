...
pub async fn start(state: AppState) {
    ...
    let mut capture_tick = interval(Duration::from_secs(
        state.config.capture.worker_interval_seconds,
    ));
    ...
    loop {
        ...
        _ = capture_tick.tick() => {
          if let Err(err) =
              captures::process_next_capture_export_request(&state.db, &state.config.capture).await
          {
